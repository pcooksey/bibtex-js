		...
		for (var index in keys) {
			...
			if (key == "BIBTEXRAW") {
				...
				tpl.find("a." + key.toLowerCase()).each(function() {
					if (!$(this).attr("href")) {
						$(this).attr("href", self.fixValue(value));
					}
				});
			}
		}
	...
	this.createStructure = function(structure, output, entries, level) {
		...
		if (groupChild.length) {
			...
			for (val in values) {
				...
				//Add the header for the group
				newStruct.children("." + sectionbibtexkey.toLowerCase()).first().prepend("<h" + (level + 2) + " class='" + groupName + "' id=\"" + sectionNameValue + "\">" + sectionNameTitle + "</h" + (level + 2) + ">");
	...
	this.checkEntry = function(entry, word) {
		...
		if (word[0] != "@") {
			entry.find("span:not(.noread)").each(
				function() {
					if ($(this).text().search(new RegExp(word, "i")) > -1 && entry.is(":visible")) {
						found = true;
			...
			if (arrayStr.length < 2) {
				entry.find("span:not(.noread)." + arrayStr[0]).each(
					function() {
						if ($(this).text().search(new RegExp(strings[1], "i")) > -1 && entry.is(":visible")) {
							found = true;
							return false; //Break out of loop
						}
					});
