...
function BibtexDisplay() {
	...
	this.getInitials = function(string) {
		return string.split(" ").map((namePart) => namePart.substring(0, 1).toUpperCase()).join(".&nbsp;") + ".";
	}
		...
		if (format.find("span:not(a)").length) {
			for (i = 0; i < searchLength; i++) {
				...
				author.find("span:not(a)").each(function() {
					var authorPartKey = $(this).attr('class').toUpperCase();
					var authorPartIndex = Format[authorPartKey];
					var authorPart = name[authorPartIndex];

					// Check if value is empty
					if (authorPart != "") {
						if (authorPartKey == "FIRST_INITIAL") {
							authorPart = that.getInitials(authorPart);
						}

						$(this).html($(this).html() + authorPart);
					} else {
						$(this).remove();
					}
				});
